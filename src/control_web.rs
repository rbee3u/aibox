...
struct ConfigFileResponse {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    custom_provider: Option<crate::config_model::CustomProviderState>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
async fn reveal_config_file(
    ...
    blocking(move || {
        ...
        let visual = if !request.current && request.file == selected.agent.main_config_file() {
            ...
            match text {
                Some(text) => match config::visual_config_state(
                    &selected,
                    request.config.as_deref().unwrap_or_default(),
                    text,
                ) {
                    Ok(state) => ConfigVisualResult {
                        options: Some(state.options),
                        custom_provider: state.custom_provider,
                        error: None,
                    },
...
async fn save_config_file(
    ...
    mutate_blocking(state, move || {
        let saved = config::save_config_file_with_linked(
            ...
            request.visual_options.as_deref(),
...
struct ConfigVisualResult {
    options: Option<Vec<config::VisualConfigOptionState>>,
    custom_provider: Option<crate::config_model::CustomProviderState>,
    error: Option<String>,
}
...
fn config_file_response(
    ...
    ConfigFileResponse {
        ...
        custom_provider: visual.custom_provider,
