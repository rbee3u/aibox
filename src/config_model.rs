...
#[derive(Clone, Debug, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct VisualProviderInput {
    pub(crate) included: bool,
...
#[derive(Clone, Debug, Serialize)]
pub(crate) struct VisualProviderState {
    pub(crate) included: bool,
    pub(crate) name: String,
    pub(crate) base_url: String,
    pub(crate) request_proxy_route: bool,
...
pub(crate) struct VisualConfigState {
    pub(crate) fields: Vec<VisualFieldState>,
    pub(crate) provider: Option<VisualProviderState>,
}
...
pub(crate) fn visual_fields(agent: AgentKind, content: &str) -> Result<VisualConfigState> {
    ...
    Ok(VisualConfigState { fields, provider })
}
...
pub(crate) fn render_visual_main(
    ...
    provider_input: Option<&VisualProviderInput>,
) -> Result<String> {
        ...
        if input.included {
            ...
            if !field.enum_values.is_empty() {
                ...
                if !field.enum_values.contains(&value) && original_value != Some(value) {
                    bail!(
                        "Visual Config Field {} must use a supported enum value",
                        input.path
                    );
                }
            }
