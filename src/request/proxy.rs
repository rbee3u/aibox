...
//! advances chunk by chunk alongside SSE indexing (`request_sse`) and Model
//! Protocol Summary observation (`request_interpretation`).
//!
...
use crate::request::interpretation::{BodyContentCoding, ProtocolObserver, body_content_coding};
use crate::request::model::{
    DiagnosticMetadata, ErrorKind, ErrorMetadata, Outcome, ProtocolFamily, ProtocolSummary,
    RecordedHeader, RequestMetadata, ResponseMetadata, ResponseModeValue, ResponseSource,
    SummaryMetadata, TimingMetadata, utc_now,
};
use crate::request::reporter::RequestReporter;
#[cfg(test)]
use crate::request::sse::is_first_token_data;
use crate::request::sse::{ObservedSseEvent, PrefixSniff, SseIndexer, SsePrefixSniffer};
use crate::request::store::{
    FORMAT_VERSION, NewRequest, ObservedRequest, RequestLocator, RequestStore, RuntimeMeasurements,
    SummaryHandle, offset_ns,
};
...
type UpstreamFuture<'a, T> = Pin<Box<dyn Future<Output = T> + Send + 'a>>;
...
impl RequestGuard {
    ...
    fn with_reporter(mut self, reporter: Option<RequestReporter>) -> Self {
...
fn recorded_headers(headers: &HeaderMap) -> Vec<RecordedHeader> {
