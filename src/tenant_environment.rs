...
use crate::component::TenantEnvironmentComponents;
...
pub(crate) fn build_command(
    ...
    components: TenantEnvironmentComponents,
) -> Vec<OsString> {
...
pub(crate) fn build_debug_command(components: TenantEnvironmentComponents) -> Vec<OsString> {
    build_command(
        &[
            OsString::from("/bin/bash"),
...
"#,
        ...
        let command = build_command_for_home(
            &[probe.into_os_string()],
            home.path().as_os_str(),
            TenantEnvironmentComponents::for_tests(true, true, true, true, true),
        );
        let output = Command::new(&command[0])
            .args(&command[1..])
            .env("HOME", home.path())
...
"#,
            ...
            let command = build_command_for_home(
                &[probe.into_os_string()],
                home.path().as_os_str(),
                TenantEnvironmentComponents::for_tests(false, false, true, false, false),
            );
            let output = Command::new(&command[0])
                .args(&command[1..])
                .env("HOME", home.path())
...
export UV_PYTHON_BIN_DIR='{uv}'\n",
    ...
    fn defaults_are_injected_only_for_the_installed_owning_component() {
        ...
        let cases: [(TenantEnvironmentComponents, &[&str]); 6] = [
            (TenantEnvironmentComponents::default(), &[]),
            (
                TenantEnvironmentComponents::for_tests(true, false, false, false, false),
                &["NPM_CONFIG_PREFIX"],
            ),
            (
                TenantEnvironmentComponents::for_tests(false, true, false, false, false),
                &["DISABLE_AUTOUPDATER"],
            ),
            (
                TenantEnvironmentComponents::for_tests(false, false, true, false, false),
            ...
            (
                TenantEnvironmentComponents::for_tests(false, false, false, true, false),
                &["CARGO_HOME", "RUSTUP_HOME"],
            ),
            (
                TenantEnvironmentComponents::for_tests(false, false, false, false, true),
                &["GOPATH", "GOROOT"],
            ),
        ];
