...
pub(crate) use crate::config_model::{CodexAuthInspection, VisualConfigState, VisualFieldState};
use crate::config_model::{
    NamedConfigDefinition, VisualAuthInput, VisualFieldInput, VisualProviderInput,
    inspect_codex_auth, render_visual_auth, render_visual_main, visual_fields,
};
...
pub(crate) fn save_config_file_with_linked(
    ...
    let (path, mode, content) = if current {
        ...
        let content = if let Some(fields) = visual {
            ...
            let original = std::str::from_utf8(&before.content)
                .with_context(|| format!("Named Config {file} is not valid UTF-8"))?;
            render_visual_main(selected.agent, original, fields, visual_provider)?.into_bytes()
    ...
    let linked = if !current
        ...
        && visual_provider.is_some_and(|provider| provider.included)
