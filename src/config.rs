    ...
    for candidate in candidates {
        let action = match classify_auth(
            ...
            AuthContent::ChatGpt(target) => {
                ...
                if target.last_refresh < source_credentials.last_refresh {
                    PlannedAction::Write {
                        path: candidate.path,
                        mode: candidate.mode,
                    }
                } else if target.last_refresh == source_credentials.last_refresh {
                    if target.value == source_credentials.value {
                        PlannedAction::Report(PropagationOutcome::Unchanged)
                    } else {
                        PlannedAction::Report(PropagationOutcome::Conflict {
                            last_refresh: target.last_refresh_text,
                        })
                    }
                } else {
                    PlannedAction::Report(PropagationOutcome::Newer {
...
fn execute_auth_propagation(plan: AuthPropagationPlan) -> AuthPropagationReport {
...
impl AuthPropagationReport {
    fn counts(&self) -> PropagationCounts {
...
pub fn create_named_config(selected: &TenantAgent, config: &str) -> Result<()> {
    ...
    let layout = inspect_named_config_directory(selected, config)?;
    match layout {
        Some(layout) if layout.complete(selected) => {
        ...
        Some(layout) => {
            ...
            prospective_auth = match selected.agent.native_auth_file() {
        ...
        None => {
            prospective_main = selected.agent.config_template().to_string();
            prospective_auth = selected.agent.config_auth_template().map(str::to_string);
        }
    }
