...
pub(crate) struct InstalledComponentSnapshot {
    node: bool,
    claude: bool,
    python: bool,
    rust: bool,
...
impl InstalledComponentSnapshot {
    #[cfg(test)]
...
pub(crate) fn inspect_tenant_environment_components(
    home: &Path,
) -> (InstalledComponentSnapshot, Vec<String>) {
    let mut installed = InstalledComponentSnapshot::default();
...
fn install_runtime_component_with_mode(
    ...
    crate::execution::runspec::reject_colon_in_bind_source("Tenant Home", &home)?;
    let run_args = crate::execution::runspec::assemble_component_run_args(&home);
