...
use super::python::{expected_python_platform, python_launcher_wrapper};
use super::runtime::{
    CLAUDE_INSTALLER, CODEX_INSTALLER, GO_INSTALLER, NODE_INSTALLER, PYTHON_INSTALLER,
    RUST_INSTALLER, capture_user_shell_profiles, install_runtime_component_with,
    restore_user_shell_profiles,
};
use super::rust_go::rustup_proxy_paths;
use super::*;
...
#[test]
fn component_wire_names_are_closed_and_stable() {
    for kind in ComponentKind::ALL {
        let encoded = serde_json::to_string(&kind).unwrap();
        assert_eq!(encoded, format!("\"{}\"", kind.name()));
        assert_eq!(
            serde_json::from_str::<ComponentKind>(&encoded).unwrap(),
            kind
        );
    }
    assert!(serde_json::from_str::<ComponentKind>("\"future\"").is_err());
}
...
fn python_inspection_keeps_historical_minor_launchers_safe_and_usable() {
    ...
    symlink(
        &historical,
        tenant.home_dir().join(".python/bin/python3.12"),
    )
    .unwrap();
    ...
    let docker = crate::docker::DockerCli::isolated(
        ...
        [
            ...
            (
                ...
                tenant.home_dir().to_path_buf().into_os_string(),
            ),
        ],
    );
