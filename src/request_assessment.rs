...
pub(crate) fn diagnostic_findings(
    ...
    collect_request_findings(summary, &mut findings);
...
fn collect_request_findings(summary: &SummaryMetadata, findings: &mut Vec<AssessmentFinding>) {
...
fn finding_sort_key(finding: &AssessmentFinding) -> (u8, u8, u128) {
    ...
    let source = if finding.source == AssessmentSource::Request
        && matches!(
            finding.kind.as_str(),
