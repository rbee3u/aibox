...
fn detail_projection_keeps_chat_order_and_tool_activity() {
    ...
    assert!(path.exists());
    assert!(matches!(records[0], DetailRecord::Message(ref message) if message.role == "user"));
    assert!(
        matches!(records[1], DetailRecord::Message(ref message) if message.role == "assistant")
    );
    assert!(matches!(records[2], DetailRecord::Tool(ref tool) if tool.name == "read_file"));
    assert!(matches!(records[3], DetailRecord::Tool(ref tool) if tool.status == "completed"));
    assert!(
...
fn codex_detail_projects_messages_tools_and_hidden_internal_records() {
    ...
    assert!(matches!(
        &records[1],
        DetailRecord::Message(message) if message.role == "user" && message.text == "inspect this"
    ));
    assert!(matches!(
        &records[2],
        DetailRecord::Message(message) if message.role == "assistant" && message.text == "I will inspect it."
    ));
    assert!(matches!(
        &records[3],
        DetailRecord::Tool(tool) if tool.name == "read_file" && tool.status == "started"
    ));
    assert!(matches!(
        &records[4],
        DetailRecord::Tool(tool) if tool.call_id.as_deref() == Some("call-1") && tool.status == "completed"
    ));
    assert!(matches!(
        &records[5],
...
fn evidence_reads_utf8_and_base64_and_rejects_stale_or_hidden_entries() {
    ...
    .unwrap();
    assert_eq!(visible.encoding, "utf-8");
