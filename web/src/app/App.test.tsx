...
describe("Console App", () => {
  ...
  it("renders the complete resource catalog in the sidebar", async () => {
    ...
    const expected = [
      ...
      ["Codex docs", "https://developers.openai.com/codex/cli", "openai"],
  ...
  it("renders the brand and tagline as an inline lockup with a decorative separator", async () => {
    mockControlApi();
    render(<App />);

    await screen.findByRole("region", { name: "Service status" });
    const brandCopy = screen.getByText("AIBox").parentElement;
    expect(brandCopy).toHaveTextContent("AIBox·Put AI in a Box");
    expect(within(brandCopy!).getByText("·")).toHaveAttribute("aria-hidden", "true");
    expect(within(brandCopy!).getByText("Put AI in a Box")).toBeInTheDocument();
  ...
  it("persists the desktop sidebar preference and defaults invalid values to expanded", async () => {
    ...
    expect(screen.getByText("AIBox").closest("[title]")).toHaveAttribute(
      "title",
      "AIBox · Put AI in a Box",
    );
    await waitFor(() =>
