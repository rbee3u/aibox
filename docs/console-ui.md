...
and dirty guards cover every navigation path: in-app and history navigation
share one `Unsaved changes` dialog with Cancel, Discard, and Save; only unload
falls back to the native prompt. A Visual/Raw switch with a draft is not a
leave — its dialog is `Switch to Raw?`/`Switch to Visual?` and says the edits
exist only in the editor being left. Use Last Application and Config
Drift language; never describe an Active Config. Credential Propagation remains
a Host Codex Current Config action, offered like Apply: a quiet `Propagate`
