...
import { ComponentGlyph } from "@/features/tenants/components/ComponentGlyph";
import {
...
interface ComponentRowItemProps {
  ...
  onMenuPosition: (position: { top: number; left: number }) => void;
  onOpenMenu: (kind: ComponentKind | null) => void;
...
export function ComponentRowItem({
  ...
  function positionMenu(element: HTMLElement) {
    onMenuPosition(componentMenuCoordinates(element.getBoundingClientRect(), model.menuWidth));
  }
          ...
          <div className={styles.componentSplitAction}>
            ...
            <ActionButton
              ...
              onClick={(event) => {
                ...
                onOpenMenu(menuOpen ? null : row.kind);
              }}
              onKeyDown={(event) => {
              ...
              createPortal(
                ...
                >
                  <button
                    ...
                    role="menuitem"
                    onKeyDown={(event) => {
                      if (event.key === "Tab") onOpenMenu(null);
                    }}
                    onClick={() => {
                      onOpenMenu(null);
                      onOpenSpecificVersion();
