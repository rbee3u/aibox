...
export type TenantKey = "host" | `managed:${string}`;

export function tenantKeyOf(row: TenantRow): TenantKey {
 ...
 */
export function fallbackTenantKey(rows: TenantRow[]): TenantKey | null {
  const fallback =
...
export function tenantLocation(key: TenantKey | null): URLSearchParams {
