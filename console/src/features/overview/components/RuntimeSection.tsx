...
import { ChevronDown, Hammer, LoaderCircle, RefreshCw } from "lucide-react";
    ...
    <>
      {operationRunning && operation && (
        <span className={styles.operationState} title={operation.kind}>
          <LoaderCircle className="spin" size={iconSize.xs} /> {operation.kind}
        </span>
      )}
      <div className={inlineCacheless ? styles.buildActions : styles.buildSplit}>
        ...
        >
          <Hammer size={iconSize.xs} aria-hidden="true" /> Build
        </ActionButton>
        {inlineCacheless ? (
...
function CachelessBuildButton({
  ...
  return (
    <ActionButton disabled={disabled} aria-describedby={describedBy} onClick={() => onBuild(true)}>
      <RefreshCw size={iconSize.xs} aria-hidden="true" /> Build without cache
    </ActionButton>
  );
}
