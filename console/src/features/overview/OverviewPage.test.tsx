...
describe("OverviewPage", () => {
  ...
  it("uses the shared module and resource icons throughout the topology", async () => {
    ...
    const tree = await openTopology();
    expect(tree.querySelector('[data-tree-icon="service"] svg')).toHaveClass("lucide-box");
    expect(tree.querySelector('[data-tree-icon="configs"] svg')).toHaveClass("lucide-file-sliders");
    expect(tree.querySelector('[data-tree-icon="sessions"] svg')).toHaveClass(
      "lucide-messages-square",
    );
    expect(tree.querySelector('[data-tree-icon="config"] svg')).toHaveClass(
      "lucide-file-code-corner",
    );
    const codex = tree.querySelector<HTMLElement>('[data-tree-icon="codex"] [data-icon="openai"]');
    const claude = tree.querySelector<HTMLElement>(
      '[data-tree-icon="claude"] [data-icon="claude"]',
    );
    expect(codex?.style.getPropertyValue("--brand-icon-size")).toBe("16px");
    expect(claude?.style.getPropertyValue("--brand-icon-size")).toBe("16px");
  });
