...
import type { SessionDetailMeta, SessionDetailStats, SessionListData } from "@/api/sessions";
...
import {
  ...
  fakeApi,
  sessionQuery,
} from "@/test/managementTestSupport";
...
describe("SessionPage", () => {
  ...
  it("keeps user messages separately and groups adjacent activity while preserving order", async () => {
    const streamSessionDetail = vi.fn((_path: string, handlers: SessionDetailHandlers) => {
  ...
  it("offers Jump to latest when a long Conversation finishes loading at the beginning", async () => {
    ...
    const streamSessionDetail = vi.fn(async (_path: string, handlers: SessionDetailHandlers) => {
      handlers.onMessage({
        entry_ids: ["message-1"],
        role: "user",
        timestamp: firstSession.start_ts,
